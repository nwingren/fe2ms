...
The second demo [windturbine_demo.py](demos/windturbine_demo.py) computes the monostatic RCS for a wind turbine rotor constructed as a fiberglass shell with an internal structure of fiberglass spars and air. It uses the mesh [rotor.msh](demos/rotor.msh) which was generated in FreeCAD and Gmsh. Note that this demo is more time and resource heavy to run than that of the coated sphere as the problem is larger and has multiple right-hand sides. The mesh is generated by the script [create_mesh_rotor.py](demos/create_mesh_rotor.py) using the STEP files [rotor_air.step](demos/rotor_air.step) and [rotor_glass.step](demos/rotor_glass.step) exported by FreeCAD.
